...
> **DB 스키마**: [../db/데이터베이스 스키마.md](../db/데이터베이스%20스키마.md) 참조
...
1. ✅ Frontend: Home → Practice → Challenge → Result 화면 + 타자 엔진
2. ✅ Backend: TextItem API + TypingSession 저장/조회 API
3. ✅ Frontend: 기록 화면(History 연습/챌린지 탭) + 홈페이지 대시보드
4. ✅ PWA: manifest
5. ⏳ Capacitor: Android 먼저 빌드 연결 → iOS

---

## 8) 코드 리뷰 및 품질 개선 (✅ 2026-02-19 완료)

> 상세: [코드 리뷰 및 품질 개선 보고서](코드%20리뷰%20및%20품질%20개선%20보고서.md)

프로젝트 전체(Backend 9앱 + Frontend 6페이지 + 인프라)를 대상으로 코드 리뷰 실시.
51건 이상 이슈 발견 → 6단계 우선순위로 전량 수정.

| Phase | 분류 | 건수 | 주요 내용 |
|-------|------|------|----------|
| 1 | 보안 | 5 | API 권한, import 검증, 프로필 보호, SECRET_KEY |
| 2 | 데이터 정합성 | 5 | F() atomic, select_for_update, timezone 통일 |
| 3 | 프론트엔드 버그 | 9 | 순환 참조, stale closure, WPM 계산, API 연동 |
| 4 | 성능 | 5 | N+1 쿼리, DB 랜덤, export 페이지네이션 |
| 5 | 인프라 | 6 | Docker 보안, nginx 헤더, gunicorn |
| 6 | 코드 품질 | 7 | console.log 정리, 404 라우트, AbortController |

---

## 9) 가상 키보드 + 손가락 가이드 (✅ 2026-02-20 완료)

> 상세: [가상 키보드 및 손가락 가이드 구현 보고서](가상%20키보드%20및%20손가락%20가이드%20구현%20보고서.md)

연습 페이지(PracticePage)와 챌린지 페이지(ChallengePage)에 가상 키보드 + 손가락 가이드 UI 추가.
새 파일 12개, 수정 4개.

| 구분 | 내용 |
|------|------|
| 키보드 | HTML/CSS Grid 5행 QWERTY, 한/영 라벨 전환, 다음 키 하이라이트, 누른 키 표시 |
| 손가락 가이드 | 좌/우 손 SVG, 활성 손가락 색상 하이라이트, 8손가락+엄지 색상 코딩 |
| 한글 IME | 유니코드 음절 분해(초/중/종성), 복합 자모 분해, 두벌식 자모→물리키 매핑 |
| 키스트로크 큐 | targetText→키스트로크 배열 변환, userInput 기반 소비 위치 추적 |
| 토글 설정 | 키보드 ON/OFF, 손가락 가이드 ON/OFF, 키입력 표시 ON/OFF |
| 반응형 | 모바일/태블릿 대응 (키 크기 축소, 서브라벨 숨김) |

---

## 관련 문서

- [프로젝트 마스터](../프로젝트%20마스터문서.md): 전체 현황/인덱스
- [DB 스키마](../db/데이터베이스%20스키마.md): 테이블 정의서 (SoT)
- [연습 모드 확장](../feature/연습모드%20확장%20기능.md): 연습 모드 요구사항
- [추천 기능 로드맵](../roadmap/추천%20기능%20로드맵.md): Phase 1~3 로드맵
- [코드 리뷰 보고서](코드%20리뷰%20및%20품질%20개선%20보고서.md): 2026-02-19 전체 코드 리뷰 결과
- [가상 키보드 보고서](가상%20키보드%20및%20손가락%20가이드%20구현%20보고서.md): 2026-02-20 키보드+가이드 구현
